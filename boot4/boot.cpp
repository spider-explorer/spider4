...
static QString prepareMain(QSplashScreen &splash)
{
    ...
    JNetworkManager nm;
    QLocale locale;
    QString spider4Json = nm.getBatchAsText(QUrl("https://raw.githubusercontent.com/spider-explorer/spider4/main/spider.json"));
    qDebug().noquote() << spider4Json;
    QJsonParseError error;
    QJsonDocument jsonDoc = QJsonDocument::fromJson(spider4Json.trimmed().toUtf8(), &error);
    ...
    QString dlPath = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
                     QString("/.spider-explorer4/.install/%1.7z").arg(version);
    qdebug_line();
    ...
    QString installDir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
                         QString("/.spider-explorer4/.install/%1").arg(version);
    QString junctionDir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
                          QString("/.spider-explorer4/.install/current");
    qdebug_line();
